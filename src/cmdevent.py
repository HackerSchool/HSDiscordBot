...
class Event(SmartScrollable):
    PATTERN1 = "((?:mon|tues|wednes|thurs|fri|satur|sun)day) +at +(\d+(?:\:\d+)?) *(pm|am)?"
    PATTERN2 = "(tomorrow|today) +at +(\d+(?:\:\d+)?) *(pm|am)?"
    PATTERN3 = "(\\d\\d?)[\\-\\/](\\d\\d?)[\\-\\/](\\d\\d\\d\\d) *(?:at)? *(\\d\\d?(?:\\:\\d\\d)?) *(am|pm)?"
    ...
    def parse_date(string):
        ...
        if string == "now":
            ...
            match = re.match(__class__.PATTERN1, string)
            if match is not None:
                weekday, time, half = match.groups()
                delta = (__class__.WEEKDAYS.index(weekday) - datetime.datetime.now().weekday()) % 7
                time = time.split(":")
                if len(time) == 1:
                ...
                try:
                    ...
                    next_date = next_date.replace(hour=hours, minute=minutes, second=0, microsecond=0)
                except ValueError:
                    return None
                return next_date
            
            match = re.match(__class__.PATTERN3, string)
            if match is not None:
                ...
                if hours < 12 and half == "pm":
                    hours += 12
                try:
                    date = datetime.datetime(int(year), int(month), int(day), hours, minutes)
